...
export function NotificationBell(){
  const {user}=useAuth(),qc=useQueryClient(),nav=useNavigate(),[open,setOpen]=useState(false),[pushPermission,setPushPermission]=useState(()=>pushService.permission()),[pushEnabled,setPushEnabled]=useState(false);
  ...
  const enablePush=useMutation({mutationFn:pushService.enable,meta:{successMessage:'Order push notifications enabled.'},onSuccess:()=>{setPushPermission('granted');setPushEnabled(true)}});
  useEffect(()=>{if(!userId||pushPermission!=='granted')return;void pushService.enable().then(()=>setPushEnabled(true)).catch(()=>setPushEnabled(false))},[pushPermission,userId]);
  if(!user)return null;

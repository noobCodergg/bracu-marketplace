...
export function RealProductDetails() {
    ...
    const originalPrice = regularPrice(product, variant);
    ...
    <div className="grid gap-10 lg:grid-cols-[1.2fr,.8fr]">
      ...
      <aside className="card h-fit p-6 lg:sticky lg:top-24"><div className="flex items-baseline justify-between"><div className="flex items-baseline gap-2"><b className="text-3xl text-emerald-700">৳{price}</b>{price<originalPrice&&<span className="text-lg text-stone-400 line-through">৳{originalPrice}</span>}</div><span className="text-sm font-semibold text-emerald-700">Order any quantity</span></div>
        {!!product.variants?.length && <div className="mt-6"><label className="label">Choose a variant</label><div className="mt-2 grid gap-2">{product.variants.map((item, index) => <button type="button" key={item.id} onClick={() => { setVariantIndex(index); setQuantity(1); }} className={`flex items-center justify-between rounded-xl border p-3 text-left ${index === variantIndex ? 'border-brand-600 bg-brand-50' : 'border-stone-200'}`}><span><b>{[item.color, item.size].filter(Boolean).join(' · ')}</b>{item.sku && <small className="ml-2 text-stone-400">{item.sku}</small>}</span><span><b className="text-emerald-700">৳{sellingPrice(product, item)}</b>{item.discountPrice!=null&&item.discountPrice<item.price&&<small className="ml-2 text-stone-400 line-through">৳{item.price}</small>}<small className="ml-2 text-emerald-700">Available</small></span></button>)}</div></div>}
...
export function RealCart() {
    ...
    return <div className="container-x py-6 sm:py-12"><h1 className="text-4xl font-extrabold">Your cart</h1>{items.length ? <div className="mt-5 grid min-w-0 gap-5 sm:mt-8 sm:gap-7 lg:grid-cols-[1.1fr,.9fr]"><section className="min-w-0 space-y-3">{items.map(item => { const key = itemKey(item.food.id, item.variant?.id); const price = sellingPrice(item.food, item.variant); return <div className="card grid min-w-0 grid-cols-[5rem,1fr] gap-3 p-3 sm:grid-cols-[7rem,1fr,auto] sm:gap-4 sm:p-4" key={key}><img src={item.food.image} alt={item.food.name} className="h-20 w-20 rounded-xl object-cover sm:h-24 sm:w-28"/><div className="min-w-0"><h2 className="line-clamp-2 font-extrabold">{item.food.name}</h2>{item.variant && <p className="truncate text-sm text-brand-700">{[item.variant.color, item.variant.size].filter(Boolean).join(' · ')}</p>}<p className="text-sm text-stone-500">৳{price} each</p><div className="mt-3 flex flex-wrap items-center gap-2"><button aria-label="Decrease quantity" className="grid h-9 w-9 place-items-center rounded-lg border" onClick={() => setQuantity(key, item.quantity - 1)}><Minus size={16}/></button><b className="min-w-5 text-center">{item.quantity}</b><button aria-label="Increase quantity" className="grid h-9 w-9 place-items-center rounded-lg border" onClick={() => setQuantity(key, item.quantity + 1)}><Plus size={16}/></button><button className="ml-1 text-sm font-bold text-red-600" onClick={() => remove(key)}>Remove</button></div></div><b className="col-start-2 row-start-2 text-right sm:col-start-3 sm:row-start-1">৳{price * item.quantity}</b></div>; })}</section><aside className="card min-w-0 h-fit p-4 sm:p-6"><h2 className="text-xl font-extrabold">Delivery details</h2><label className="label mt-4">Address</label><textarea className="field min-h-20" value={address} onChange={e => setAddress(e.target.value)}/><label className="label mt-4">Phone</label><input type="tel" className="field" value={phone} onChange={e => setPhone(e.target.value)}/><div className="mt-4 grid gap-3 sm:grid-cols-2"><label><span className="label">Date</span><input type="date" min={new Date().toISOString().slice(0, 10)} className="field" value={date} onChange={e => setDate(e.target.value)}/></label><label><span className="label">Time</span><input type="time" className="field" value={time} onChange={e => setTime(e.target.value)}/></label></div><label className="label mt-4">Instructions</label><textarea className="field min-h-20" value={instructions} onChange={e => setInstructions(e.target.value)}/><div className="mt-5 border-t pt-5"><label className="label">Seller coupon</label><div className="grid gap-2 sm:grid-cols-[1fr,auto]"><input className="field min-w-0 uppercase" value={couponCode} onChange={e => { setCouponCode(e.target.value.replace(/\s/g, '')); setCoupon(null); redeem.reset(); }} placeholder="Enter coupon code"/><Button variant="secondary" disabled={!couponCode || redeem.isPending} onClick={() => redeem.mutate()}>{redeem.isPending ? 'Checking...' : 'Apply'}</Button></div>{redeem.isError && <p className="mt-2 text-sm font-semibold text-red-600">{(redeem.error as Error).message}</p>}{coupon && <p className="mt-2 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">{coupon.code}: {coupon.discountPercent}% off {coupon.seller}</p>}</div><div className="mt-5 space-y-2 border-t pt-5 text-sm"><div className="flex justify-between"><span>Subtotal</span><span>৳{subtotal}</span></div>{coupon && <div className="flex justify-between text-emerald-700"><span>Coupon discount</span><span>-৳{discount}</span></div>}<div className="flex justify-between border-t pt-3 text-lg font-extrabold"><span>Total</span><span>৳{subtotal - discount}</span></div></div><Button className="mt-5 w-full" disabled={!!user && (address.trim().length < 10 || phone.trim().length < 10 || !date || !time || checkout.isPending)} onClick={() => user ? checkout.mutate() : nav('/login', { state: { from: { pathname: '/cart' } } })}>{user ? (checkout.isPending ? <Spinner /> : 'Place order') : 'Sign in to checkout'}</Button>{checkout.isError && <p className="mt-3 text-sm font-semibold text-red-600">{(checkout.error as Error).message}</p>}</aside></div> : <div className="mt-8"><Empty title="Your cart is empty" body="Choose a product and variant from the marketplace."/></div>}</div>;
}

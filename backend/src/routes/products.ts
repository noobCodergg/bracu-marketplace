...
productRouter.get('/seller/:sellerId/availability',async(req,res,next)=>{try{res.setHeader('Cache-Control','no-store');res.setHeader('CDN-Cache-Control','no-store');const seller=await UserModel.findOne({_id:req.params.sellerId,role:'SELLER'}).select('acceptingOrders').lean();if(!seller){res.status(404).json({success:false,message:'Seller not found'});return}res.json({success:true,message:'Seller availability loaded',data:seller.acceptingOrders??true})}catch(error){next(error)}});

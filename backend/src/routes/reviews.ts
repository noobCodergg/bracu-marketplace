...
reviewRouter.get('/product/:productId',publicResponseCache(30_000),async(req,res,next)=>{try{const reviews=await ReviewModel.find({productId:req.params.productId}).sort({createdAt:-1}).limit(100).lean();res.json({success:true,message:'Reviews loaded',data:reviews.map(view)})}catch(error){next(error)}});
